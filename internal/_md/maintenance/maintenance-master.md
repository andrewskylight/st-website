# Master Maintenance Sheet

## MONITOR🧐:
- Battery Alarms during anchoring
- Saildrive 2 (starboard side):
    - output seal
    - saildrive showed seawater a number of times;
    - prioritize for seal change
- Yamaha STORE: remember we have credit there!

## CURRENT ITEMS🔧:
- Repair shore power cable (notify Andrew if cannot be repaired)
- Figure out if the low battery alarms during anchoring are related to the anchor mechanism itself or it's due to aging battteries or some other reason altogether
- Adjust Autopilot so that p70 control head displays properly when the rudder is in neutral position (currently it has port offset)
- Fix EPRB device with batteries
- Engine / saildrive oil change at 1000hr
- Check & maybe repalce anode on the water tank (Room 1, under bed)
- Redo green labels for room numbers & compartments
- Change three way switch near the fresh water pump
- Trim up sail so it does not damage the sunbrella

## NO TOUR DAY⛵:
- 💧 Repair water tank; Reinforce weak points
- 🎛️ Add control wiring for bilge pumps

## UPHOLSTERY (Bertha)☂️:
- Sunbrella: repair fabric
- Repair trampoline (in storage)
- Repair white cusions on the roof

## CAPTURE (not prioritized, but recorded)📋:
- Hull repair: Port Bow Side ... damage from Pig Lovers dock
- Storage bins on the side of the hulls (white)
- Windows R3, R4 - reseal; they are leaking
- Compartment M2 - possible water intrusion again :(
- Carpentry: close top of the bar area (winches, old air conditioner)
- Refill diving tanks

## WAITING FOR PARTS⏳:
## IDEAS💡:
